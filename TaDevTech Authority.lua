...
shared.LoaderTitle = "LuaMain Hub";
...
--     end
-- end);
local v14 = loadstring(game:HttpGet("https://raw.githubusercontent.com/tadevtechathority/UIMainFluent/main/UIMainCode.lua"))();
local v15 = v14:CreateWindow({
    Title = "LuaMain Hub",
    SubTitle = "TaDevTechAuthority",
    TabWidth = 160,
    Theme = "Light",
    Acrylic = false,
...
local AttackSpeedConfig = {
    ["Normal"] = { delay = 0.6, fireserver = 0.4 },
    ["Fast"]   = { delay = 0.000000000001, fireserver = 0.000000000000000002 },
    ["Super Fast"] = { delay = 0.000000000000000001, fireserver = 0 }
}
local CurrentAttack = AttackSpeedConfig["Normal"]

local v118 = v16.Setting:AddDropdown("DropdownSelectAttackSpeed", {
    Title = "Fast Attack",
...
v118:OnChanged(function(value)
    CurrentAttack = AttackSpeedConfig[value] or AttackSpeedConfig["Normal"]
end)

v118:SetValue("Normal")

CurrentAttack = AttackSpeedConfig["Normal"]
...
if Options and Options.ToggleFastAttack then
    Options.ToggleFastAttack:SetValue(true)
else

    pcall(function() ToggleFastAttack:SetValue(true) end)
end
_G.FastAttack = true 


if typeof(ToggleAutoFarm) == "function" or ToggleAutoFarm then
    ToggleAutoFarm:OnChanged(function(val)
        AutoFarmMasDevilFruit = val
    end)
    
    if Options and Options.ToggleAutoFarm then
        Options.ToggleAutoFarm:SetValue(false) 
    else
        pcall(function() ToggleAutoFarm:SetValue(false) end)
    end
...
spawn(function()
    while true do
        ...
        if not AutoFarmMasDevilFruit then
            ...
            wait(CurrentAttack.delay)
        end

        if _G.FastAttack then
            local f = d(game.Players.LocalPlayer)
            if #f > 0 then
                b:FireServer(CurrentAttack.fireserver)
                for g,i in next, f do
                    c:FireServer(f[g][2], f)
                end
            end
        end
    end
